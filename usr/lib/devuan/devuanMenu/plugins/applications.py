...
class pluginclass( object ):
    ...
    @print_timing
    def __init__( self, devuanMenuWin, toggleButton, de ):
        self.devuanMenuWin = devuanMenuWin
       ...
       # self.searchButton.connect( "button_release_event", self.SearchWithButton )
        try:
        # GSettings stuff
            self.settings = EasyGSettings( "com.devuan.devuanmenu.plugins.applications" )
    ...
    def refresh_apt_cache(self):
        if self.useAPT:
            os.system("mkdir -p %s/.devuan/devuanMenu/" % home)
            os.system("/usr/lib/devuan/devuanMenu/plugins/get_apt_cache.py > %s/.devuan/devuanMenu/apt.cache &" % home)
    ...
    def get_panel(self):
        ...
        for applet in applet_list:
            ...
            if "applet-iid" in keys:
                ...
                if iid is not None and iid.find("DevuanMenu") != -1:
    ...
    def checkDevuanMenuFolder( self ):
        if os.path.exists( os.path.join( os.path.expanduser( "~" ), ".devuan", "devuanMenu", "applications" ) ):
            return True
        try:
            os.makedirs( os.path.join( os.path.expanduser( "~" ), ".devuan", "devuanMenu", "applications" ) )
            return True
        except:
            pass
    ...
    def add_search_suggestions(self, text):
        ...
        suggestionButton.set_image("/usr/lib/devuan/devuanMenu/search_engines/google.ico")
        ...
        suggestionButton.set_image("/usr/lib/devuan/devuanMenu/search_engines/wikipedia.ico")
        self.applicationsBox.add(suggestionButton)
        self.suggestions.append(suggestionButton)
        ...
        suggestionButton.set_image("/usr/lib/devuan/devuanMenu/search_engines/dictionary.png")
        self.applicationsBox.add(suggestionButton)
        self.suggestions.append(suggestionButton)
        ...
        #self.devuanMenuWin.SetPaneColors( [  self.last_separator ] )
    ...
    def add_apt_filter_results(self, keyword):
        try:
            ...
            command = "cat %(home)s/.devuan/devuanMenu/apt.cache" % {'home':home}
    ...
    def searchPopup( self, widget=None, event=None ):
        ...
        img = Gtk.Image()
        img.set_from_file('/usr/lib/devuan/devuanMenu/search_engines/google.ico')
        ...
        img = Gtk.Image()
        img.set_from_file('/usr/lib/devuan/devuanMenu/search_engines/wikipedia.ico')
        menuItem.set_image(img)
        ...
        img = Gtk.Image()
        img.set_from_file('/usr/lib/devuan/devuanMenu/search_engines/dictionary.png')
        menuItem.set_image(img)
        ...
        img = Gtk.Image()
        img.set_from_file('/usr/lib/devuan/devuanMenu/search_engines/software.png')
        menuItem.set_image(img)
        menuItem.connect("activate", self.search_devuan_software)
        menu.append(menuItem)

        menuItem = Gtk.ImageMenuItem(_("Find Tutorials"))
        img = Gtk.Image()
        img.set_from_file('/usr/lib/devuan/devuanMenu/search_engines/tutorials.png')
        menuItem.set_image(img)
        menuItem.connect("activate", self.search_devuan_tutorials)
        menu.append(menuItem)

        menuItem = Gtk.ImageMenuItem(_("Find Hardware"))
        img = Gtk.Image()
        img.set_from_file('/usr/lib/devuan/devuanMenu/search_engines/hardware.png')
        menuItem.set_image(img)
        menuItem.connect("activate", self.search_devuan_hardware)
        menu.append(menuItem)

        menuItem =Gtk.ImageMenuItem(_("Find Ideas"))
        img = Gtk.Image()
        img.set_from_file('/usr/lib/devuan/devuanMenu/search_engines/ideas.png')
        menuItem.set_image(img)
        menuItem.connect("activate", self.search_devuan_ideas)
        menu.append(menuItem)

        menuItem = Gtk.ImageMenuItem(_("Find Users"))
        img = Gtk.Image()
        img.set_from_file('/usr/lib/devuan/devuanMenu/search_engines/users.png')
        menuItem.set_image(img)
        menuItem.connect("activate", self.search_devuan_users)
        menu.append(menuItem)
    ...
    def search_devuan_tutorials(self, widget):
        text = self.searchEntry.get_text()
        text = text.replace(" ", "%20")
        os.system("xdg-open \"http://community.devuan.com/index.php/tutorial/search/0/" + text + "\" &")
        self.devuanMenuWin.hide()

    def search_devuan_ideas(self, widget):
        text = self.searchEntry.get_text()
        text = text.replace(" ", "%20")
        os.system("xdg-open \"http://community.devuan.com/index.php/idea/search/0/" + text + "\" &")
        self.devuanMenuWin.hide()

    def search_devuan_users(self, widget):
        text = self.searchEntry.get_text()
        text = text.replace(" ", "%20")
        os.system("xdg-open \"http://community.devuan.com/index.php/user/search/0/" + text + "\" &")
        self.devuanMenuWin.hide()

    def search_devuan_hardware(self, widget):
        text = self.searchEntry.get_text()
        text = text.replace(" ", "%20")
        os.system("xdg-open \"http://community.devuan.com/index.php/hardware/search/0/" + text + "\" &")
        self.devuanMenuWin.hide()

    def search_devuan_software(self, widget):
        text = self.searchEntry.get_text()
        text = text.replace(" ", "%20")
        os.system("xdg-open \"http://community.devuan.com/index.php/software/search/0/" + text + "\" &")
        self.devuanMenuWin.hide()
    ...
    def favoritesBuildLauncher( self, location ):
        try:
            ...
            if favButton.appExec:
                ...
                favButton.connect( "clicked", lambda w: self.devuanMenuWin.hide() )

                self.devuanMenuWin.setTooltip( favButton, favButton.getTooltip() )
    ...
    def buildFavorites( self ):
        try:
            if (not os.path.exists(home + "/.devuan/devuanMenu/applications.list")):
                os.system("mkdir -p " + home + "/.devuan/devuanMenu/applications")
                os.system("cp /usr/lib/devuan/devuanMenu/applications.list " + home + "/.devuan/devuanMenu/applications.list")

            applicationsFile = open ( os.path.join( os.path.expanduser( "~" ), ".devuan", "devuanMenu", "applications.list" ), "r" )
    ...
    def favoritesSave( self ):
        try:
            self.checkDevuanMenuFolder()
            appListFile = open( os.path.join( os.path.expanduser( "~"), ".devuan", "devuanMenu", "applications.list" ) , "w" )
            ...
            msgDlg = Gtk.MessageDialog( None, gtk.DialogFlags.MODAL, Gtk.MessageType.ERROR, Gtk.ButtonsType.OK, _("Couldn't save favorites. Check if you have write access to ~/.devuan/devuanMenu")+"\n(" + e.__str__() + ")" )
            msgDlg.run();
            msgDlg.destroy();
            ...
            if addedCategories:
                ...
                for item in addedCategories:
                    try:
                        ...
                        self.devuanMenuWin.setTooltip( item["button"], item["tooltip"] )
            ...
            if addedApplications:
                ...
                for item in addedApplications:
                    ...
                    if item["button"].appExec:
                        self.devuanMenuWin.setTooltip( item["button"], item["button"].getTooltip() )
                        item["button"].connect( "button-press-event", self.menuPopup )
                        item["button"].connect( "focus-in-event", self.scrollItemIntoView )
                        item["button"].connect( "clicked", lambda w: self.devuanMenuWin.hide() )
