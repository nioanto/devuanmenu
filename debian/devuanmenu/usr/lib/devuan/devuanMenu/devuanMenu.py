...
class MainWindow( object ):
    ...
    def __init__( self, toggleButton, settings, keybinder ):
        ...
        self.icon = "/usr/lib/devuan/devuanMenu/visualisation-logo.png"

        self.toggle = toggleButton
        # Load UI file and extract widgets
        builder = Gtk.Builder()
        builder.add_from_file(os.path.join( self.path, "devuanMenu.glade" ))
        ...
        plugindir = os.path.join( os.path.expanduser( "~" ), ".devuan/devuanMenu/plugins" )
    ...
    def SetupDevuanMenuBorder( self, defaultStyle = None ):
            ...
            if plugin != "newpane":
                try:
                    ...
                    if X.pluginclass.__init__.func_code.co_argcount == 1:
                        ...
                        # pass devuanMenu and togglebutton instance so that the plugin can use it
    ...
    def loadTheme( self ):
        ...
        self.SetupDevuanMenuBorder( defaultStyle )
        self.SetHeadingStyle( self.headingsToColor )
...
class MenuWin( object ):
    def __init__( self, applet, iid ):
        self.applet = applet
        self.settings = Gio.Settings.new("com.devuan.devuanmenu")
        ...
        self.mainwin.window.set_name("devuanmenu") # Name used in Gtk RC files
        self.applyTheme()
    ...
    def createPanelButton( self ):
        ...
        if os.path.exists("/etc/devuan/info"):
            import commands
            tooltip = commands.getoutput("cat /etc/devuan/info | grep DESCRIPTION")
    ...
    def showAboutDialog( self, action, userdata = None ):
        ...
        about.set_name("devuanMenu")
        import commands
        version = commands.getoutput("/usr/lib/devuan/common/version.py devuanmenu")
      ...
      #  about.set_authors( ["Clement Lefebvre <clem@devuan.com>", "Lars-Peter Clausen <lars@laprican.de>"] )
        about.set_translator_credits(("translator-credits") )
        #about.set_copyright( _("Based on USP from S.Chanderbally") )
        about.set_logo( GdkPixbuf.Pixbuf.new_from_file("/usr/lib/devuan/devuanMenu/icon.svg") )
        about.connect( "response", lambda dialog, r: dialog.destroy() )
        about.show()


    def showPreferences( self, action, userdata = None ):
#               Execute( "mateconf-editor /apps/devuanMenu" )
        Execute( os.path.join( PATH, "devuanMenuConfig.py" ) )
    ...
    def create_menu(self):
        ...
        action = Gtk.Action("DevuanMenuPrefs", _("Preferences"), None, "gtk-preferences")
        action.connect("activate", self.showPreferences)
        action_group.add_action(action)
        action = Gtk.Action("DevuanMenuEdit", _("Edit menu"), None, "gtk-edit")
        action.connect("activate", self.showMenuEditor)
        action_group.add_action(action)
        action = Gtk.Action("DevuanMenuReload", _("Reload plugins"), None, "gtk-refresh")
        action.connect("activate", self.mainwin.RegenPlugins)
        action_group.add_action(action)
        action = Gtk.Action("DevuanMenuAbout", _("About"), None, "gtk-about")
        action.connect("activate", self.showAboutDialog)
        action_group.add_action(action)
        action_group.set_translation_domain ("devuanmenu")
...
MatePanelApplet.Applet.factory_main("DevuanMenuAppletFactory", True,
